  ...
  import {
    Container, Typography, Box, Card, CardMedia, Chip, Button, Table, TableBody, TableCell, Snackbar, Alert,
  ...
  const GameDetails = () => {
    ...
    const [alertSeverity, setAlertSeverity] = useState('success'); // success, info, error, etc.
    ...
    const handleAddToWishlist = async () => {
      try {
        ...
        if (addToWishlist.fulfilled.match(resultAction)) {
          setAlertMessage(resultAction.payload.message);
          setAlertSeverity('success');
        } else if (resultAction.payload && resultAction.payload.exists) {
          // Verifica si el payload indica que el juego ya está en la wishlist
          setAlertMessage('El juego ya está en la wishlist.');
          setAlertSeverity('info');
        } else {
          setAlertMessage('Error al añadir a la wishlist.');
          setAlertSeverity('error');
        }
  
        setAlertOpen(true); // Abre el Snackbar después de configurar el mensaje
      } catch (err) {
        console.error("Error al añadir a la wishlist:", err);
        setAlertMessage('No se pudo agregar a la wishlist.');
        setAlertSeverity('error');
        setAlertOpen(true);
      }
    };
          ...
          <Container maxWidth="md">
            ...
            >
              ...
              <Box mt={4}>
                ...
                anchorOrigin={{ vertical: 'top', horizontal: 'center' }} // Posiciona el alert en la parte superior y centrado
              >
